...
class TestExtractDataLogging():
    ...
    @pytest.mark.it('test extract data creates correct logger_used')
    def test_extract_data_uses_correct_logger(self,test_s3, caplog):
        with caplog.at_level(logging.INFO):
            extract_data(test_s3)
            print(caplog)
            print("^^^^", caplog.text)
        assert "wooo" in caplog.text
        assert "hoo" in caplog.text
    ...
    @pytest.mark.skip
    @pytest.mark.it('test extract data outputs correct info message')
    def test_extract_data_outputs_correct_info_message(self,test_client):
        with self.assertLogs() as captured:
            extract_data(test_client)
        self.assertIn("is this message there>",captured.output[0])
    @pytest.mark.skip    
    @pytest.mark.it('test extract data outputs correct number of info message')
